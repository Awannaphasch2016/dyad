  ...
  private async inspectCurrentRepository(
    ...
    if (health.unmergedFiles.length > 0) {
      return {
        ...
        message:
          "This project has unresolved file conflicts. Resolve them outside wewebplus, then check again.",
      };
    }
    if (health.operationInProgress) {
      return {
        ...
        message: `A Git ${health.operationInProgress} is still in progress. Finish or cancel it outside wewebplus, then check again.`,
      };
    }
    if (!health.branch) {
      return {
        ...
        message:
          "This project is not on a named branch. Return it to a branch outside wewebplus, then check again.",
      };
    }
