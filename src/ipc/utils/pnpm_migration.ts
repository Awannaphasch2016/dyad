...
export async function applyPnpmVersionMigration({
  ...
  if (!pnpmSupport.available || !pnpmSupport.version) {
    throw new DyadError(
      "pnpm is not available, so the project cannot be migrated. Restart wewebplus and try again.",
  ...
  if (
    ...
    throw new DyadError(
      `The available pnpm (${pnpmSupport.version}) is older than pnpm ${COMPATIBLE_PNPM_LOCKFILE_MAJOR}, so the project cannot be migrated. Restart wewebplus and try again.`,
      DyadErrorKind.External,
    );
  }
  ...
  try {
    await simpleSpawnWithDeniedPnpmBuildSelfHeal({
      ...
      cwd: appPath,
      successMessage:
        "Reinstalled dependencies with the wewebplus-managed pnpm",
