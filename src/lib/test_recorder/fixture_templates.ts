...
const NEON_BETTER_AUTH_FIXTURE: string[] = [
  ...
  ``,
  `/**`,
  ` * Sign in the wewebplus-provisioned test user by driving the app's own Better Auth`,
  ...
  ` *`,
  ` * wewebplus provisions an isolated user per test case and injects its credentials`,
  ...
  `  if (!email || !password) {`,
  `    throw new Error(`,
  `      "DYAD_TEST_USER_EMAIL / DYAD_TEST_USER_PASSWORD are not set. Run from wewebplus's Tests panel and read credentials inside a test or beforeEach, after the isolated user is provisioned; they are unavailable at module scope or in beforeAll.",`,
  `    );`,
  `  }`,
...
const SUPABASE_PASSWORD_FIXTURE: string[] = [
  ...
  ``,
  `/**`,
  ` * Sign in the wewebplus-provisioned Supabase test user via the password grant,`,
  ` * then seed supabase-js's session into localStorage before the app loads so`,
  ` * it boots authenticated. wewebplus injects the project URL, anon key, and the`,
  ...
  `    throw new Error(`,
  `      "Supabase test credentials are not set. Run from wewebplus's Tests panel and read credentials inside a test or beforeEach, after the isolated user is provisioned; they are unavailable at module scope or in beforeAll.",`,
  `    );`,
  `  }`,
